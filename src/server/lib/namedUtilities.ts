...
const getCloneCommands = (depReq: DeployRequest): string[] => {
    if (depReq.repos.length === 1) {
        return [
            `git clone -b ${depReq.repos[0].branch ?? 'master'} --recurse-submodules --single-branch https://github.com/${depReq.repos[0].username}/${
                depReq.repos[0].repo
            }.git ${depReq.deployId}`
        ];
    }
    return depReq.repos.map(
        (repo) =>
            `git clone -b ${repo.branch ?? 'master'}  --recurse-submodules --single-branch https://github.com/${repo.username}/${repo.repo}.git ${depReq.deployId}/${
                repo.repo
            }`
    );
};
